...
def Bayes(eMailContent):
    eMailContent = Filter_text(eMailContent)
    eMailContentCount = sorted(Count(eMailContent).items(), key=lambda x: x[1], reverse=True)
    # 提取前15个词作计算条件概率，代入贝叶斯联合公式
    if len(eMailContentCount) >= 15:
        r = 15
    # 如果长度不够，就取总词数
    else:
        r = len(eMailContentCount)
    P = []
    for n in range(r):
        word = eMailContentCount[n][0]
...
spam = ''
for x in range(1, 26):
    f = open('email/ham/' + str(x) + '.txt', 'r', errors='ignore')
    health = health + f.read() + ' '
    f.close()
    f = open('email/spam/' + str(x) + '.txt', 'r', errors='ignore')
    spam = spam + f.read() + ' '
    f.close()
health = Filter_text(health)
# spam字符串第一个是空格，不好看
spam = Filter_text(spam)[1:]
# 转化为有序的字典
...
def main():
    print('是垃圾邮件概率：', Bayes(open('testFile.txt', 'r', errors='ignore').read()))
